...
(function () {
  ...
  if (hero) {
    ...
    if (loopId && wide.matches && !reduced.matches) {
      // Il loop parte dopo il primo disegno: la locandina non deve mai aspettarlo.
      window.addEventListener('load', function () {
        setTimeout(function () {
          var f = document.createElement('iframe');
          f.title = 'Showreel in riproduzione muta';
          f.setAttribute('aria-hidden', 'true');
          f.setAttribute('tabindex', '-1');
          f.allow = 'autoplay; encrypted-media';
          f.src = 'https://www.youtube-nocookie.com/embed/' + loopId +
            '?autoplay=1&mute=1&controls=0&loop=1&playlist=' + loopId +
            '&playsinline=1&modestbranding=1&rel=0&disablekb=1&iv_load_policy=3';
          f.addEventListener('load', function () {
            setTimeout(function () { f.classList.add('is-ready'); }, 400);
          });
          media.appendChild(f);
        }, 700);
      });
